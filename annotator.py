...
def create_image_with_points():
    ...
    if len(points['pupil']) >= 3:
        ...
        p_x, p_y, p_r, p_res = leastsq_circle(pupil_x_points, pupil_y_points)
    ...
    if len(points['iris']) >= 3:
        ...
        i_x, i_y, i_r, i_res = leastsq_circle(iris_x_points, iris_y_points)
...
def select_image_folder():
    global images, image_index, previous_image_button, next_image_button, pupil_button, iris_button, \
                    current_img, canvas, delete_button, image_name_only, folder_path, image_mult
    if len(images) != 0:    
        data_file_prev = os.path.join(folder_path, image_name_only[image_index].split('.')[0] + '.json')
        global points, p_x, p_y, p_r, i_x, i_y, i_r
        data = {}
        data['points'] = points
        data['image_mult'] = image_mult
    ...
    Label(root, text=images[image_index]).grid(row=1, columnspan=10)
    points['pupil'] = []
    points['iris'] = []
...
def previous_image():
    global images, image_index, current_img, canvas, points, image_mult
    if image_index > 0:
        data_file_prev = os.path.join(folder_path, image_name_only[image_index].split('.')[0] + '.json')
        data = {}
        data['points'] = points
...
def next_image():
    global images, image_index, current_img, canvas, points, p_x, p_y, p_r, i_x, i_y, i_r, image_mult
